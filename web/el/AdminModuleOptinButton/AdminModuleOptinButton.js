...
export function AdminModuleOptinButton({children}) {
  ...
  const creatorBalance = creatorAlgoAccount.data
    ? creatorAlgoAccount.data.amount - creatorAlgoAccount.data['min-balance']
    : 0;
  ...
  const onClick = useCallback(
    event => {
      ...
      async function run() {
        ...
        try {
          const txnUnsigned = await getTxnUnsigned({algod, from: walletAddress, appIndex: ADMIN_ID});
    ...
    [provider, creatorAlgoAccount, algod, walletAddress, ADMIN_ID]
  );
  ...
  return (
    <>
      <StatusButton
        ...
        success={creatorAlgoAccount.isFetched && Boolean(adminModuleState)}
        disabled={
          creatorAlgoAccount.isLoading || isLoading || (creatorAlgoAccount.isFetched && Boolean(adminModuleState))
        }
        blocked={creatorBalance < 1000}
        title={creatorBalance < 1000 ? 'Insufficient balance' : null}
        error={errorMessage}>
        {children}
      </StatusButton>
      {txnId ? (
...
AdminModuleOptinButton.propTypes = {
